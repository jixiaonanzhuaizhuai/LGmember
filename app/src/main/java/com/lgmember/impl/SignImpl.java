...
public class SignImpl extends HttpApi {
    ...
    public void getActivityCode(String activitySignCode, final ActivityCodeBusiness.ActivityCodeResulHandler handler, Context context){
        //判断没有网络应该如何处理
        if (!app.isNetWorkEnable(context)) {
            handler.onNetworkDisconnect();
        }
        ...
        MyOkHttp mMyOkhttp = new MyOkHttp(okHttpClient());
        mMyOkhttp.get()
                .url(Common.URL_ACTIVITY_CODE)
                .addParam("code",activitySignCode)
                .tag(this)
                .enqueue(new JsonResponseHandler() {
                    @Override
                    public void onSuccess(int statusCode, JSONObject response) {
                ...
                .enqueue(new JsonResponseHandler() {
                    ...
                    public void onSuccess(int statusCode,
                        ...
                        try {
                            int code = response.getInt("code");
                            if (code == 0) {
                                handler.onUploadRecordSuccess("上传成功");
                            } else {
                                handler.onError(code);
                            }
                        } catch (JSONException e) {
