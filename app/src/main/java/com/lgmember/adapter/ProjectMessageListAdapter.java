...
public class ProjectMessageListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            TextView state = (TextView)view.findViewById(R.id.txt_create_time);
        ...
        if (str == null){
            vh.iv_img.setImageResource(R.mipmap.image1);
        }else {
            final String result = str.substring(str.lastIndexOf(","),str.length());
            vh.iv_img.setImageBitmap(StringUtil.base64ToBitmap(result));
        }
        vh.title.setText(""+projectMessage.getTitle());
        vh.create_time.setText(""+projectMessage.getStart_time());
        vh.state.setText(""+projectMessage.getState());
